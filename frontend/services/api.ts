...
// Leave this relative in production: nginx serves the UI, REST API, and
// WebSocket from one public origin. An override remains available for local
// development against a separately running backend.
const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || '/api';
const apiUrl = (path: string) => new URL(`${API_BASE_URL}${path}`, window.location.origin).toString();

export async function fetchTrains(query: string = '', limit: number = 20, offset: number = 0): Promise<{ total: number; trains: Train[] }> {
  try {
    const url = new URL(apiUrl('/trains'));
...
export async function fetchTrainById(trainId: string): Promise<Train | null> {
  try {
    const res = await fetch(apiUrl(`/trains/${trainId}`), { cache: 'no-store' });
...
export async function fetchTrainStations(trainId: string): Promise<ScheduleItem[]> {
  try {
    const res = await fetch(apiUrl(`/trains/${trainId}/stations`), { cache: 'no-store' });
    if (!res.ok) return [];
    return await res.json();
  } catch (err) {
...
export async function fetchTrainRoute(trainId: string): Promise<[number, number][]> {
  try {
    const res = await fetch(apiUrl(`/trains/${trainId}/route`), { cache: 'no-store' });
    if (!res.ok) return [];
...
export async function fetchTrainPrediction(trainId: string): Promise<ETAPrediction | null> {
  try {
    const res = await fetch(apiUrl(`/trains/${trainId}/prediction`), { cache: 'no-store' });
    if (!res.ok) return null;
    return await res.json();
  } catch (err) {
...
export async function fetchNetworkStatus(): Promise<NetworkStatus | null> {
  try {
    const res = await fetch(apiUrl('/network/status'), { cache: 'no-store' });
    if (!res.ok) return null;
    return await res.json();
  } catch (err) {
...
export async function fetchAlerts(limit: number = 20): Promise<Alert[]> {
  try {
    const res = await fetch(apiUrl(`/alerts?limit=${limit}`), { cache: 'no-store' });
    if (!res.ok) return [];
    return await res.json();
  } catch (err) {
...
export async function submitOperationalDelay(trainId: string, category: string, delayMinutes: number, reason: string) {
  try {
    const res = await fetch(apiUrl(`/trains/${encodeURIComponent(trainId)}/operational-delay`), {
      method: 'POST',
...
export async function controlSimulation(action: 'start' | 'pause' | 'reset') {
  try {
    const res = await fetch(apiUrl(`/simulation/${action}`), { method: 'POST' });
    return await res.json();
  } catch (err) {
...
export async function extractIncidentFromVoice(message: string, trainId?: string) {
  try {
    const res = await fetch(apiUrl('/assistant/incident-extract'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function acknowledgeIncident(incidentId: string) {
  try {
    const res = await fetch(apiUrl(`/incidents/${incidentId}/acknowledge`), { method: 'POST' });
    return await res.json();
  } catch (err) {
...
export async function escalateIncident(incidentId: string) {
  try {
    const res = await fetch(apiUrl(`/incidents/${incidentId}/escalate`), { method: 'POST' });
    return await res.json();
  } catch (err) {
...
export async function resolveIncident(incidentId: string) {
  try {
    const res = await fetch(apiUrl(`/incidents/${incidentId}/resolve`), { method: 'POST' });
    return await res.json();
  } catch (err) {
